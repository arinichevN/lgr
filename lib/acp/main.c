...
void acp_parsePackI1S1(char *buf, I1S1List *list, size_t list_max_size) {
    ...
    int n = sprintf(format, "%%d_%%%lus", LINE_SIZE);
    if (n <= 0) {
        return;
    }
    while (list->length < list_max_size) {
...
int acp_sendBuf(char *buf, size_t buf_size, Peer *peer) {
    return sendBuf((void *) buf, acp_packlen(buf, buf_size), *(peer->fd), (struct sockaddr *) (&peer->addr), peer->addr_size);
}

int acp_sendBufArrPackI1List(char cmd, size_t buf_size, const I1List *data, Peer *peer) {
...
int acp_sendBufArrPackI2List(char cmd, size_t buf_size, const I2List *data, Peer *peer) {
    char q[LINE_SIZE], buf[buf_size];
    int i;
    if (!acp_initBuf(buf, buf_size)) {
        return 0;
...
int acp_sendBufArrPackS2List(char cmd, size_t buf_size, const S2List *data, Peer *peer) {
    char q[LINE_SIZE], buf[buf_size];
    int i;
    if (!acp_initBuf(buf, buf_size)) {
        return 0;
...
int acp_sendBufArrPackS1List(char cmd, size_t buf_size, const S1List *data, Peer *peer) {
    char q[LINE_SIZE], buf[buf_size];
    int i;
    if (!acp_initBuf(buf, buf_size)) {
        return 0;
...
int acp_sendBufPack(char *buf, char qnf, char *cmd_str, size_t buf_size, Peer *peer) {
...
int acp_sendStrPack(char qnf, char *cmd, size_t buf_size, Peer *peer) {
    char buf[buf_size];
...
int acp_recvI2(I2List *list, char qnf, char *cmd, size_t buf_size, size_t list_max_size, int fd) {
    ...
    if (!crc_check(buf, sizeof buf)) {
#ifdef MODE_DEBUG
        fputs("acp_recvI2: crc_check() failed\n", stderr);
#endif
        return 0;
    }
...
int acp_sendSMS(Peer *peer, char *phone, char *message, size_t udp_buf_size) {
    if (lockPeer(peer)) {
        S2 di[1];
        memcpy(di[0].p0, phone, LINE_SIZE);
        memcpy(di[0].p1, message, LINE_SIZE);
        S2List data = {di, 1};
        if (!acp_sendBufArrPackS2List(ACP_CMD_GWU59_SEND_SMS, udp_buf_size, &data, peer)) {
#ifdef MODE_DEBUG
...
int acp_makeCall(Peer *peer, char *phone, size_t udp_buf_size) {
    if (lockPeer(peer)) {
        S1List data = {phone, 1};
        if (!acp_sendBufArrPackS1List(ACP_CMD_GWU59_RING, udp_buf_size, &data, peer)) {
#ifdef MODE_DEBUG
